...
// Clear session and cookies if logged in and user visits the login page
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) {
    session_destroy();
    
    // Remove "Remember Me" cookies by setting them to expire in the past
    setcookie("email", "", time() - 3600, "/");
    setcookie("password", "", time() - 3600, "/");
    session_start(); // Restart session after destruction
}
...
// Check if "Remember Me" cookies exist to pre-fill email and password
$email = isset($_COOKIE['email']) ? $_COOKIE['email'] : "";
$password = isset($_COOKIE['password']) ? $_COOKIE['password'] : "";

// Auto-login using cookies
if (!empty($email) && !empty($password)) {
    $query = "SELECT * FROM users WHERE email = ? OR contact_number = ? OR username = ?";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($password == $user['password']) {
            ...
            $query = "UPDATE users SET last_activity = '$current_time' WHERE id = ?";
            ...
            if ($user['id'] != -1) {
                header("location: dashboard_user.php?id=" . $user['id']);
            } else {
...
if (isset($_POST["submit"])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($pass == $user['password']) {
            ...
            // If "Remember Me" is selected, set cookies
            if ($remember) {
          ...
          <div class="row">
              ...
              <div class="card card-plain mt-8">
                ...
                <div class="card-body">
                  <form role="form" method="POST">
                    ...
                    <?php if (!empty($error)): ?>
                    <div class="text-danger text-sm mt-2" id="error-message"><?php echo $error; ?></div>
                    <?php endif; ?>
            ...
            <div class="col-md-6">
              <div class="oblique position-absolute top-0 h-100 d-md-block d-none me-n8">
                <div class="oblique-image bg-cover position-absolute fixed-top ms-auto h-100 z-index-0 ms-n6" style="background-image:url('../assets/img/olym.jpeg')"></div>
              </div>
            </div>
          </div>
        </div>
